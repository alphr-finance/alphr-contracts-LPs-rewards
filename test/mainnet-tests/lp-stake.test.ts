...
describe('Reward :: test reward contract', () => {
  ...
  describe('Reward :: test stake method', async () => {
    it('test stake method', async () => {
      ...
      const nftHolderAddress = '0x72d4d59f31d0cb4a4213a5dc81fc0108241ecc71';
  ...
  describe('Reward :: test stake within alphr token', async () => {
    ...
    before('mint nft token for pair alphr/WETH9', async () => {
      ...
      before('get ERC20 tokens', async () => {
        ...
        const alphr = (await ethers.getContractAt(
          'IERC20',
          ALPHR_TOKEN
        )) as IERC20;
        await alphr.approve(nonFungibleManager.address, MaxUint128);
        await alphr.approve(user.address, MaxUint128);
        ...
        await alphr
          .connect(alphrTokenHolder)
          .transfer(user.address, utils.parseEther('5'));
        ...
        await weth.approve(user.address, MaxUint128);
    ...
    it('test stake method', async () => {
      ...
      // await nonFungibleManager.connect(user).mint({
      //   token0: WETH9,
      //   token1: ALPHR_TOKEN,
      //   tickLower: getMinTick(TICK_SPACINGS[FeeAmount.MEDIUM]),
      //   tickUpper: getMaxTick(TICK_SPACINGS[FeeAmount.MEDIUM]),
